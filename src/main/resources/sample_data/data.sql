...
INSERT INTO app_user (
    id, email, password,
    active, locked, enabled, credentials_non_expired,
...
-- === PROFILES ===
INSERT INTO profile (
    id, user_id, name, preferred_weight_unit, deleted,
    created_at, created_by,
    last_modified_at, last_modified_by,
    version
)
SELECT
    1, 1, 'Coach Fox', 'KG', false,
    NOW(), 'DEV_SEED',
    NOW(), 'DEV_SEED',
    0
...
INSERT INTO coach_client (
    id, coach_id, client_id, started_at, active,
    created_at, created_by,
    last_modified_at, last_modified_by,
    version
)
...
-- === EXERCISE_MUSCLE_GROUP (NO AUDIT) ===
...
WHERE NOT EXISTS (
    ...
    WHERE exercise_id = tmp.exercise_id AND muscle_group = tmp.muscle_group
);

-- === CYCLE ===
INSERT INTO cycle (
    id, author_id, trainee_id, name, description, level,
    created_at, created_by,
    last_modified_at, last_modified_by,
    version
)
SELECT
    1, 1, 2, 'Hypertrophy Cycle', '8-week hypertrophy block', 'INTERMEDIATE',
    NOW(), 'DEV_SEED',
    NOW(), 'DEV_SEED',
    0
...
-- === WEEK ===
INSERT INTO week (
    id, cycle_id, order_number, completed, note,
    created_at, created_by,
    last_modified_at, last_modified_by,
    version
)
...
-- === WORKOUT ===
INSERT INTO workout (
    id, name, author_id, trainee_id, description, is_template, weight_unit, note,
    created_at, created_by,
    last_modified_at, last_modified_by,
    version
)
...
WHERE NOT EXISTS (
    SELECT 1 FROM workout_tag
    WHERE workout_id = tmp.workout_id AND tag_id = tmp.tag_id
);

-- === WEEK_WORKOUT ===
INSERT INTO week_workout (
    id, workout_id, week_id, day_of_the_week, completed,
    created_at, created_by,
    last_modified_at, last_modified_by,
    version
)
...
INSERT INTO workout_exercise (
    id, order_number, workout_id, exercise_id, workout_exercise_type, note,
    created_at, created_by,
    last_modified_at, last_modified_by,
    version
)
